...
//  requests
process.on('request:error', function (uri, e) {
  ...
  console.error.apply(console, [(new Date()).toJSON(), 'ERROR: ', e.status, uri]);
...
process.on('request:success', function (uri, status) {
  console.log.apply(console, [(new Date()).toJSON(), 'SUCCESS: ', status, uri]);
});

...
module.exports = function createApiDefinition (name, pathName, path, api) {
  ...
  if (Array.isArray(api.streams)) {
    ...
    queryAndRespond = lateBind(queryAndStreamResponse, name, api.type, api.query, api.streams.concat(responseStream));
  }
    ...
    queryAndRespond = lateBind(queryAndPipeResponse, name, api.type, api.query, api.pipeline, responsePipeline);
  }
  else {
  	  throw new Error('su-apiserver:'  + apiName + ' endpoint can have either a `pipeline` Array or an Array of `streams`, IDIOT!');
  }

  args.push(queryAndRespond);

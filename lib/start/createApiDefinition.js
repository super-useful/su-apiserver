...
module.exports = function createApiDefinition (name, pathName, path, api, apiApp) {
  ...
  if (typeof path.cache.get === 'function') {
  ...
  if (Array.isArray(api.streams)) {
    ...
    args.push(lateBind(typeof path.cache.get === 'function' ? cachedOrQueryAndPipeResponse : queryAndPipeResponse, name, api.type, api.query), lateBind(responsePipeline, compose(api.pipeline)));

    if (typeof path.cache.set === 'function') {

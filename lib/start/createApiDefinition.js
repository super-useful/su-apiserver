...
var getCached = require('../interceptors/getCached');

var cacheResponse = require('../transformers/cacheResponse');
...
module.exports = function createApiDefinition (name, pathName, path, api, apiApp) {
  ...
  if (path.cache) {
    pathInterceptors.unshift(getCached);
  }
  ...
  if (Array.isArray(api.streams)) {
    ...
    if (path.cache) {
      api.pipeline.push(cacheResponse);
    }
    ...
    args.push(lateBind(path.cache ? cachedOrQueryAndPipeResponse : queryAndPipeResponse, name, api.type, api.query), lateBind(responsePipeline, compose(api.pipeline)));
  }
  else {
